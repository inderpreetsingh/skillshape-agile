...
import ClassTypeLocationRequest, { ClassTypeLocationRequestSchema } from './fields';
...
Meteor.methods({
  'classTypeLocationRequest.addRequest': function (data, subscriptionRequest) {
    ...
    if (isValid) {
      let locationRequest;
      if (!data.classTypeId) {
        locationRequest = ClassTypeLocationRequest.findOne({
          email: data.email,
      ...
      if (locationRequest) {
        ...
        const classTypeName = data.classTypeId
          ...
          : '';
        const memberLink = this.userId
          ? `${Meteor.absoluteUrl()}schools/${slug}/members?userId=${this.userId}`
          : '';
        ...
        sendRequestReceivedEmail({
          ...
          classTypeName,
          schoolPageLink,
          updateLink: updateClassLocationLink,
          memberLink,
          requestFor,
        });

        if (subscriptionRequest === 'save' || this.userId) locationRequestId = ClassTypeLocationRequest.insert(data);
        ...
        if (subscriptionRequest === 'save') {
          ...
          const subject = `Subscription for location request of ${classTypeName
            || schoolData.name}`;
  ...
  'classTypeLocationRequest.updateRequest': function (data) {
    const old = ClassTypeLocationRequest.findOne({
      userId: data.userId,
      classTypeId: data.classTypeId,
    });
    ...
    try {
      const res = ClassTypeLocationRequest.update(
        { userId: data.userId, classTypeId: data.classTypeId },
        { $set: data },
        { upsert: true },
      );
      return res;
