...
Meteor.methods({
  project_upload(fileName, fileData) {
    this.unblock();
    try {
      ...
      if (checkMyAccess({ user, viewName: 'csvUpload_Schools' })) {
        ...
        for (let i = 0; i < csvLength; i++) {
          try {
            ...
            if (
              csvdata[i].schoolName
              && csvdata[i].website
              && csvdata[i].schoolName.length > 0
              && emailRegex.test(csvdata[i].email)
            ) {
              ...
              const slocation = SLocation.findOne({
                schoolId,
                zip: csvdata[i].zip,
                $or: [{ title: csvdata[i].LocationTitle }, { address: csvdata[i].address }],
              });
              const sLocationDoc = {
              ...
              } else if (csvdata[i].address || csvdata[i].city || csvdata[i].state) {
                locationId = SLocation.insert(sLocationDoc);
              }

              const skillCategoryIds = [];
              const skillSubjectIds = [];
              const classTypeObject = {
                isPublish: true,
                filters: {
                  location: [
                    {
                      loc: {
                        ...
                        locationId,
                        title: `${sLocationDoc.state}, ${sLocationDoc.city}, ${
                          sLocationDoc.country
                        }`,
                      },
                    },
                  ],
              ...
              let { skillCategory } = csvdata[i];
              let { skillSubject } = csvdata[i];

              if (skillCategory) {
                skillCategory = skillCategory.split(',');
                for (const skill of skillCategory) {
                  let skillCategoryId;
                  const skillCategoryObject = SkillCategory.findOne({ name: skill.trim() });

                  if (skillCategoryObject) {
                    skillCategoryId = skillCategoryObject._id;
                  } else if (csvdata[i].classTypeName) {
              ...
              if (skillSubject) {
                ...
                for (const skill of skillSubject) {
                  let skillSubjectId;
                  const skillSubjectObject = SkillSubject.findOne({ name: skill.trim() });
              ...
              if (classTypeObject.name) {
                try {
                  ClassType.update(
                    {
                      schoolId,
                      name: csvdata[i].classTypeName,
                      desc: csvdata[i].classTypeDesc,
                    },
                    { $set: classTypeObject },
                    { upsert: true },
                  );
              ...
              } else if (classTimeObject.className) {
              ...
              ImportLogs.update(
                { _id: csvLogId },
                {
                  $inc: { errorRecordCount: 1, totalRecord: 1 },
                  $push: { errorRecord: csvdata[i] },
                },
              );
            }
          } catch (err) {
            ImportLogs.update(
              { _id: csvLogId },
              { $inc: { errorRecordCount: 1, totalRecord: 1 }, $push: { errorRecord: csvdata[i] } },
            );
          }
        }
  ...
  getErrorDocs(logId) {
    if (logId) {
      const log = ImportLogs.findOne({ _id: logId });
...
function CreateNewUser(email, name, firstName, lastName, schoolId) {
  if (typeof email !== 'undefined' && emailRegex.test(email)) {
    const _user = Accounts.findUserByEmail(email);
    if (!_user) {
      const userId = Accounts.createUser({
        email,
        password: email,
        profile: { firstName, lastName, schoolId: [schoolId] },
        preverfiedUser: true,
      });
