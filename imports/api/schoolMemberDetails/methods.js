    ...
    "schoolMemberDetails.editSchoolMemberDetails": function({doc_id, doc}) {
        ...
        if(doc && doc.studentWithoutEmail && doc.email) {
            ...
            if(!userRecExist) {
                ...
                let ROOT_URL = `${Meteor.absoluteUrl()}?acceptInvite=${true}&memberId=${doc_id}&schoolId=${doc.schoolId}`;
                let rejectionUrl = `${Meteor.absoluteUrl()}?rejectInvite=${true}&memberId=${doc_id}&schoolId=${doc.schoolId}`;
                let schoolData = School.findOne(doc.schoolId);
                sendEmailToStudentForClaimAsMember(user, schoolData, userRecExist, "", fromEmail, toEmail, ROOT_URL, rejectionUrl);
