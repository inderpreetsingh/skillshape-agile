...
export const formatTime = (startTime, timeZone) => {
  ...
  return moment(props || startTime).format('hh:mm A');
};

// This method will work without props..
export const formatAmPm = (startTime) => {
  if (startTime) {
    const hours = startTime.props ? startTime.props.getHours() : startTime.getHours();
...
// console.info(date, moment(date).format('DD-MM-YYYY'), ";;;;;;;;;;");
	 moment(date).format(`${monthFormat} Do, YYYY`);
export const formatDateNoYear = date => moment(date).format('MMMM DD');
...
export const formatClassTimesData = (classTimesData, hidePastDates = true) => classTimesData.map((data) => {
...
export const formatDataBasedOnScheduleType = (data, hidePastDates = true) => {
  ...
  if (data.scheduleDetails && data.scheduleType === 'oneTime') {
    classTimes = {};
    const currentSchedule = data.scheduleDetails.oneTime || data.scheduleDetails;
    let startDate; let dayOfTheWeek; let day; let startTime; let formattedTime; let timePeriod; let currentJsonData; let
      timeUnits;
    ...
    currentSchedule.forEach((item) => {
      ...
      if (classTimes[day]) {
        const existingTimes = classTimes[day];
  ...
  if (hidePastDates) {
    return removePastTimesFromSchedule(classTimes, data.scheduleType.toLowerCase(), {
