...
export class FormBuilderModal extends React.Component {
  ...
  initializeFormValues = () => {
    const { formFieldsValues } = this.props;
    const formFields = this.getFormFields();
    if (formFields) {
      for (const field of formFields) {
        const value = formFieldsValues && formFieldsValues[field.key];

        if (field.type === 'image') {
          this.refs[`${field.key}_src`].src = value || config.defaultSchoolImage;
          this.refs[field.key].value = null;
          $(this.refs[field.key]).change();
        } else {
          this.refs[field.key].value = value || null;
        }
      }
    }
  };
  ...
  getFooterButton = (type) => {
    ...
    if (type === 'Edit') {
      ...
      btnArray.push(
        <button key={type} form={formId} type="submit" className="btn btn-default">
          ADD
        </button>,
      );
    }
  ...
  onSubmit = (event) => {
    ...
    let callApi;
    const {
      modalType, tableData, formFieldsValues, parentData,
    } = this.props;
    ...
    for (const formField of formFields) {
      switch (formField.type) {
        ...
        case 'auto-select': {
          const autoSelectValue = this.refs[formField.key].getValue();
        ...
        case 'autoComplete': {
          const temp = this.refs[formField.key].getSelectedAutoCompleteValue();
          ...
          if (formField.child) {
            const temp = this.refs[formField.key].getAutoSelectValue();
    ...
    switch (field.type) {
      ...
      case 'select':
        return (
          ...
          >
            {field.defaultOption && (
              <option disabled defaultValue={field.options[0].value}>
                {field.defaultOption}
              </option>
            )}
      ...
      case 'image':
        return (
          <div className="" style={{ textAlign: 'center' }}>
            <div
              className="fileinput fileinput-new card-button text-center"
            ...
            >
              ...
              <div>
                ...
                <a
                  href="#"
                  className="btn btn-danger btn-sm btn-file fileinput-exists"
  ...
  render() {
    const { className, modalType } = this.props;
      ...
      <div className="modal fade " id="FormBuilderModal" role="dialog">
        <div className="modal-dialog" style={{ maxWidth: '650px' }}>
          <div className={`modal-content ${className}`}>
            ...
            <div className="modal-footer">{this.getFooterButton(modalType)}</div>
          </div>
        </div>
      </div>
    );
