...
import {
  packageCoverProvider, withPopUp, confirmationDialog, formatDate,
} from '/imports/util';
import ClassInterest from '/imports/api/classInterest/fields';

const MySubscriptionRender = React.lazy(() => import('./MySubscriptionRender'));
...
class MySubscription extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const updatePurchases = async (purchaseData) => {
  ...
  classDataFinder = (schoolData) => {
    ...
    const schoolId = get(schoolData, '_id', null);
    const userId = get(currentUser, '_id', null);
    const filter = { schoolId, activeUserId: userId };
    ...
    Meteor.call('schoolMemberDetails.getMemberData', filter, (err, res) => {
      ...
      Meteor.call('classInterest.findClassTypes', schoolId, userId, (err, res) => {
        if (!isEmpty(res)) state.subscriptionsData = res;
        else state.subscriptionsData = [];
  ...
  leaveSchool = () => {
    const { popUp, currentUser, userId } = this.props;
    const { selectedSchool: schoolData } = this.state;
    const { _id: schoolId, name: schoolName } = schoolData;
    const studentName = get(
      currentUser,
      'profile.firstName',
      get(currentUser, 'profile.name', 'Old Data'),
    );
    let content = '';
  ...
  render() {
    const {
      ...
      memberId,
      privacySettings,
      purchaseDataWithCover = [],
    } = this.state;
    const { isLoading, schoolData, currentUser } = this.props;
...
export default createContainer((props) => {
  ...
  if (
    ...
    && classInterestSubscription.ready()
