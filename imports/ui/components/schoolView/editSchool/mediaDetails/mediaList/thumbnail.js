...
export default class Thumbnail extends React.Component {
  ...
  getCreatorName = (thumbnailData) => {
  	if (thumbnailData && thumbnailData.creator && thumbnailData.creator.profile) {
  		const fullName = `${thumbnailData.creator.profile.firstName} ${thumbnailData.creator.profile.lastName}`;
  		return fullName;
  	}
  }
...
Taken:
              {' '}
              {moment(thumbnailData.createdAt).format('MMM Do YY, h:mm a')}
            </h4>
            <h4>
By:
              {' '}
              {this.getCreatorName(thumbnailData)}
            </h4>
          </div>
        </div>
      </div>
