...
export default class ClassTimeForm extends React.Component {
	...
	getScheduleDetails = (scheduleDetails) => {
		...
		Object.keys(temp).map((key, index)=> {
			if(!temp[key].isSeleted) {
				...
				if(_.isEmpty(temp[key].roomId)) {
					temp = null
					toastr.error("Please select any room *","Error");
					return
				}
			}
		})
	...
	validateObject = (obj) => {
  	...
  	onSubmit = (event) => {
    	...
    	const {  classTypeId, schoolId, addForm, classTimesData } = this.props;
    	const payload = {
	...
	render() {
		...
		const { scheduleDetails } = this.state;
		console.log("ClassTimeForm scheduleDetails -->>",scheduleDetails);
					...
					{ this.state.isBusy && <ContainerLoader/> }
							...
							<div style={styles.formControl}>
	      					...
	      					<div style={styles.formControl}>
				               	<div style={styles.formControlInput}>
				               		<SelectField
				               			floatingLabelText="Schedule Type *"
								        hintText="Select Schedule Type"
				               			disabled={editMode}
				               			fullWidth={true}
								        value={this.state.scheduleType}
								        onChange={this.handleSelectChange.bind(this, "scheduleType")}
								    >
								        <MenuItem value="onGoing" primaryText="Ongoing" />
								        <MenuItem value="oneTime" primaryText="One Time" />
								        <MenuItem value="recurring" primaryText="Repeating Class with Start and End Dates" />
								     </SelectField>
				               	</div>
	      					</div>
	      					{
	      						(this.state.scheduleType === "onGoing" || this.state.scheduleType === "recurring") && (
	      							<Fragment>	
	      								{ 
