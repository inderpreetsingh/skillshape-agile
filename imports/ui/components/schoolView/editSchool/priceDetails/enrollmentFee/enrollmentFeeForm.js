...
class EnrollmentFeeForm extends React.Component {
  ...
  onSubmit = (event) => {
    ...
    const {
      data, schoolId, classTypeData, popUp,
    } = this.props;
    ...
    if (
      isEmpty(payload.classTypeId)
      || !payload.currency
      || !payload.name
      || !payload.cost
      || (!this.state.noExpiration && !this.expDuration.value)
      || (!this.state.noExpiration && !expPeriod)
    ) {
      popUp.appear('alert', {
        title: 'Please Check',
        content: 'Not all required fields are complete.',
      });
      return;
    }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <EnrollmentText>{enrollmentText}</EnrollmentText>
        ...
        {this.state.error ? (
          ...
          <DialogContent>
            <form id={formId} onSubmit={this.onSubmit}>
              ...
              />
              {!this.state.includeAllClassTypes && (
                <SelectArrayInput
                  disabled={false}
