...
import { dateFriendly, getUserFullName } from '/imports/util';
...
class UsersList extends React.Component {
  ...
  handlePageClick = ({ skip }) => {
    ...
    this.getUsers({ limit: this.state.perPage, skip });
  };
              ...
              : usersData.map(user => (
                <TableRow key={user._id} selectable={false}>
                  <TableCell style={style.w150}>
                    {getUserFullName(user) || 'Unavailable'}
                  </TableCell>
                  <TableCell style={style.w211}>
                    {user.emails[0].address || 'Unavailable'}
                  </TableCell>
                  <TableCell style={style.w150}>
                    {(user.createdAt && dateFriendly(user.createdAt, 'MM/DD/YYYY'))
                          || 'Unavailable'}
                  </TableCell>
                  <TableCell style={style.w100}>
                    {(user.roles && user.roles.join(', ')) || 'Not Assigned Yet'}
                  </TableCell>
                </TableRow>
              ))}
          </UsersDetailsTable>
        </div>
        <Pagination {...this.state} pageCount={pageCount} onChange={this.handlePageClick} />
      </Fragment>
    );
  }
}
