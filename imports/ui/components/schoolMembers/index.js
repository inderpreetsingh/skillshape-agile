...
class SchoolMemberView extends Component {
  ...
  render() {
    ...
    const slug = get(this.props.params, 'slug', this.props.slug);
    const view = get(this.props, 'view', 'classmates');
    const userId = get(this.props, 'userId', null);
    const filters = { ...this.state.filters };

    if (!isUserSubsReady) return <Preloader />;
    ...
    return (
      <DocumentTitle title={get(this.props.route, 'name', 'Member Listing')}>
        <Grid
          ...
          style={{
            position: 'relative', backgroundColor: '#fff', margin: '0', width: '100%',
          }}
        >
          <DashBoardView
            filters={filters}
