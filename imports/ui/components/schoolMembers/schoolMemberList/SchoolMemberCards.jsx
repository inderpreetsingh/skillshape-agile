...
const getNormalizedMembersData = (props) => {
  const {
    view, membersByName, cardsView, handleMemberDetailsToRightPanel, superAdminId,
  } = props;
      ...
      .forEach((key) => {
        ...
        if (!isEmpty(membersByName[key])) {
          membersByName[key].forEach((data) => {
            let profile; let pic; let firstName; let
              emails;
            if (view == 'classmates') {
...
const SchoolMembersScreen = (props) => {
  ...
  return (
    <MembersGridWrapper>
      <MembersGrid cardsView={cardsView}>{membersData}</MembersGrid>
    </MembersGridWrapper>
  );
};
