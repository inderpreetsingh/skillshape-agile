...
import MemberDialogBox from '/imports/ui/components/landing/components/dialogs/MemberDetails';
...
const SchoolMemberInfo = lazy(() => import('../schoolMemberInfo'));
const SchoolAdminsList = lazy(() => import('/imports/ui/components/schoolMembers/schoolMemberList/SchoolMemberCards'));
...
class DashBoardView extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const schoolId = get(schoolData[0], '_id', null);
    const schoolName = get(schoolData[0], 'name', 'School Name');
    if (!isLoading && schoolId && activeUserId && isEmpty(memberInfo)) {
      const data = { schoolId, activeUserId };
      Meteor.call('schoolMemberDetails.getMemberData', data, async (err, memberInfo) => {
        if (!isEmpty(memberInfo)) {
          const {
            ...
            profile: {
              profile: {
                name, pic, firstName, lastName, phone,
              },
              emails,
            },
          } = memberInfo;
          const subscriptionList = await packageCoverProvider(
            get(purchaseByUserId, activeUserId, []),
          );
          const email = get(emails[0], 'address', null);
          const schoolImg = get(
            schoolData[0],
            'mainImageMedium',
            get(schoolData[0], 'mainImage', config.defaultSchoolImage),
          );
          const superAdmin = superAdminId == activeUserId;
          this.setState({
            memberInfo: {
              _id: activeUserId,
              memberId,
  ...
  _getElementHeight = elem => elem && elem.getBoundingClientRect && elem.offsetHeight;

  handleSlidingDrawerState = () => {
    if (window.innerWidth <= helpers.tablet) {
      if (!this.state.slidingDrawerState) this.setState(state => ({ ...state, slidingDrawerState: true }));
    } else if (this.state.slidingDrawerState) this.setState(state => ({ ...state, slidingDrawerState: false }));
  };
      ...
      <Grid container spacing={24}>
        ...
        <Grid item sm={12} xs={12} md={12} style={{ display: 'flex', justifyContent: 'flex-end' }}>
          ...
          <PrimaryButton
            formId="addUser"
            type="submit"
            label={`Add a New ${view == 'classmates' ? 'Member' : 'Admin'}`}
          />
          {' '}
          <PrimaryButton
            formId="cancelUser"
            label="Cancel"
            onClick={() => this.setState({ renderStudentModal: false, error: false })}
          />
        </Grid>
      </Grid>
    );
  ...
  allowAddNewMemberWithThisEmail = (event) => {
    ...
    const { isAdmin, schoolData } = this.props;
    const schoolId = schoolData[0]._id;
    const _id = (this.state.message && this.state.message.userId) || null;
    const schoolName = schoolData[0].name;
    const to = this.email.value;
    const userName = this.firstName.value;
    const payload = {};
    const adminName = get(
      Meteor.user(),
      'profile.firstName',
      get(Meteor.user(), 'profile.name'),
      'Admin',
    );
      ...
      Meteor.call(
        ...
        (err, res) => {
          ...
          if (res) {
            ...
            if (findIndex(adminsIds, o => o == _id) != -1 && isAdmin) {
  ...
  handleMemberDetailsToRightPanel = async (memberId, superAdminId) => {
    ...
    let profile;
    let pic;
    const schoolId = get(schoolData[0], '_id', '');
    let email;
    let _id;
    let superAdmin;
    const schoolName = get(schoolData[0], 'name', '');
    ...
    superAdmin = superAdminId == _id;
    pic = get(
      profile,
      'medium',
      get(profile, 'pic', get(config, 'defaultProfilePicOptimized', '')),
    );
    // memberInfo = this.state.memberInfo
    const subscriptionList = await packageCoverProvider(get(purchaseByUserId, _id, []));
  ...
  handleInput = (event) => {
    ...
    const oldMemberInfo = { ...this.state.memberInfo };
  ...
  render() {
    ...
    const schoolMemberListFilters = { ...this.state.filters };
    if (slug) {
            ...
            <Drawers height={this.state.membersViewHeight}>
              <Fragment>
                ...
                <ToggleVisibilityTablet show>
                  <FixedDrawer variant="permanent">{drawer}</FixedDrawer>
                </ToggleVisibilityTablet>
              </Fragment>
            </Drawers>

            <SchoolMemberWrapper id="school-member-wrapper">
              <Suspense fallback={<Loading />}>
                <SchoolMemberInfo
                  selectedSchoolData={find(schoolData, { _id: memberInfo.schoolId })}
                  memberInfo={memberInfo}
                  ...
                  isAdmin={isAdmin}
                  notClassmatePage={get(this.props.location, 'pathname', null) != '/classmates'}
                />
                {this.renderSchoolMedia(schoolData, memberInfo, slug)}
              </Suspense>
...
export default createContainer((props) => {
  const slug = get(props.params, 'slug', props.slug);
  let schoolData = [];
  let isLoading = true;
  let classTypeData = [];
  const filters = { ...props.filters, slug };
  ...
  let filter;
  let purchaseByUserId;
  let superAdminId;
  const subscription = Meteor.subscribe('schoolMemberDetails.getSchoolMemberWithSchool', filters);
  if (subscription && subscription.ready()) {
    classTypeData = ClassType.find().fetch();

    if (slug) {
      schoolData = School.find({ slug }).fetch();
    } else {
    ...
    const enrollmentFeeSubscription = Meteor.subscribe('enrollmentFee.getEnrollmentFee', {
      schoolId,
    });
    ...
    if (
      ...
      && enrollmentFeeSubscription.ready()
    ...
    if (!isEmpty(schoolData) && schoolData[0].admins) {
      ...
      if (schoolAdmin) {
        ...
        isAdmin = true;
        const x = findIndex(adminsIds, o => o == Meteor.userId());
        if (x == -1) {
          adminData = remove(adminsData, o => o._id == Meteor.userId());
        }
      }
    }
  }
