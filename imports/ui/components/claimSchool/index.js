...
import {
  confirmationDialog, getUserFullName, redirectToThisUrl, withPopUp,
} from '/imports/util';
...
class ClaimSchool extends ClaimSchoolBase {
  handleClaimASchool = (schoolCardData) => {
    ...
    const userEmail = get(
      user,
      'emails[0].address',
      get(user, 'user.services.google.email', 'No Email Found'),
    );
    ...
    Meteor.call('claimSchoolRequest.createClaimSchoolRequest', payload, (err, result) => {
      // Stop Loading
      this.setState({ isLoading: false });
      if (result) {
        if (result.alreadyRejected) {
          popUp.appear('alert', {
            content: 'Your request has been rejected to manage this school by school Admin.',
          });
          ...
          popUp.appear('alert', {
            content: `You are not allowed to do more than one request as you have already created request for School Name:${
              result.schoolName
            }`,
          });
        } else if (result.emailSuccess) {
          popUp.appear('success', {
            content: 'We have sent your request to school admin. We will assist you soon :)',
          });
        } else if (result.claimRequestApproved) {
          const url = `/SchoolAdmin/${schoolCardData._id}/edit`;
          const data = {
            popUp,
            title: 'Success',
            type: 'success',
            ...
            buttons: [
              { label: 'Ok', onClick: () => {}, greyColor: true },
              {
                label: 'Manage School',
                onClick: () => {
...
export default createContainer((props) => {
  ...
  const dataForSkillTypes = SkillCategory.find().fetch();
