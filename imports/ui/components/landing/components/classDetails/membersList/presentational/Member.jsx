...
import MemberExpanded from './MemberExpanded';
...
import { addInstructorImgSrc } from '/imports/ui/components/landing/site-settings';
...
const Member = (props) => {
  const { profile } = props;
  const profileSrc = props.addMember
    ? addInstructorImgSrc
    : get(profile, 'medium', get(profile, 'pic', config.defaultProfilePicOptimized));
  let name = `${get(profile, 'firstName', get(profile, 'name', 'Old Data'))} ${get(
    profile,
    'lastName',
    '',
  )}`;
  ...
  removePopUp = (popUp, classTime) => {
    popUp.appear(
      'success',
      {
        ...
        content: (
          <div>
            Successfully removed from instructor listing.
            <br />
            {' '}
            {classTime ? 'Changes will show in the Class Times Editor after saving.' : ''}
          </div>
        ),
        RenderActions: (
          <ButtonsWrapper>
          ...
          <ButtonsWrapper>
            ...
            <ButtonWrapper>
              <FormGhostButton
                ...
                onClick={() => {
                  this.handleRemove(popUp, payLoad);
                }}
                applyClose
              />
            </ButtonWrapper>
            <ButtonWrapper>
              <FormGhostButton
                label="Whole series"
                onClick={() => {
                  this.handleRemove(popUp, payLoad, 'whole');
                }}
                applyClose
              />
            </ButtonWrapper>
  ...
  handleRemove = (popUp, payLoad, from) => {
    ...
    if (!from) {
      ...
      payLoad.instructors = remove(payLoad.instructorIds, n => n != payLoad.instructorId);
      Meteor.call(
  ...
  handleMenuItemClick = (option) => {
    const { popUp, _id } = props;
    const operation = get(option, 'value', null);
            ...
            <ButtonsWrapper>
              <ButtonWrapper>
                <FormGhostButton
                  ...
                  label="Remove"
                  onClick={() => {
                    this.handleRemoveInstructor(props);
                  }}
                  applyClose
                />
              </ButtonWrapper>
    ...
    >
      <Profile>
        ...
        <DetailsWrapper type={props.type}>
          <Details>
            <SubHeading align="center" fontSize="20">
              {cutString(name, 20)}
            </SubHeading>
            {props.type !== 'student' && !props.addMember && (
              <Text align="center">
        ...
        {props.viewType === 'instructorsView' && !props.addMember && (
          <DropDownMenu
