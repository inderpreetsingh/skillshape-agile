...
import { rhythmDiv, panelColor, danger } from '/imports/ui/components/landing/components/jss/helpers';
...
class MembersListContainer extends Component {
  ...
  studentsData = (props) => {
    ...
    const { classTypeId } = (classData && classData) || {};
    ...
    if (classData) {
      get(classData, 'students', []).map((obj, index) => {
        ...
        this.setState({
          pageCount: Math.ceil(studentsIds.length / this.state.limit),
          packagesRequired,
        });
      });
      if (!isEmpty(studentsIds)) {
        const { limit, skip } = this.state;
        const limitAndSkip = { limit, skip };
        const data = {
          studentsIds, classTypeId, schoolId, classData, purchaseIds, limitAndSkip,
        };
  ...
  changePageClick = (skip) => {
    this.setState({ skip: skip.skip, isBusy: true }, () => {
      this.studentsData(this.props);
    });
  };
    ...
    Meteor.call('purchase.manageAttendance', _id, packageType, inc, (err, res) => {
      ...
      if (condition > 0 || res == undefined) {
        Meteor.call('classes.updateClassData', filter, status, _id, packageType, (err, res) => {
          if (res) {
            const newStatus = `${
              status == 'signIn' ? 'Sign In' : status == 'checkIn' ? 'Check In' : 'Sign Out'
            }`;
            popUp.appear(
              'success',
              {
                title: packageConnected
                  ? 'Package successfully purchased.'
                  : `${newStatus} successfully`,
                content: packageConnected
                  ? 'Your student can now attend class!'
                  : `Student have been successfully ${newStatus}.`,
                RenderActions: (
                  <ButtonWrapper>
                    <FormGhostButton
                      label="Ok"
                      onClick={() => {
                        !packageConnected
                          ...
                          : this.setState({ buyPackagesBoxState: false });
                      }}
                      applyClose
                    />
                  </ButtonWrapper>
    ...
    Meteor.call('classPricing.signInHandler', filter, (err, res) => {
      ...
      if (epStatus && !isEmpty(purchased)) {
              ...
              <ButtonWrapper>
                {purchased.map((obj, index) => (
                  <FormGhostButton
                    ...
                    onClick={() => {
                      this.updateClass(filter, status, obj, popUp, packageConnected);
                    }}
                    applyClose
                  />
                ))}
        ...
        popUp.appear('alert', {
          title: 'Alert',
          content:
            "Oops user don't have any package. Please use Accept payment button to accept payment and send link or else ask user to purchase one itself.",
        });
      }
      ...
      // }
        ...
        let packageType; let packagesRequired; let content; let
          title;
        if (!epStatus) {
  ...
  purchaseEnrollmentFirst = (popUp) => {
    popUp.appear('alert', {
      title: 'Alert',
      content:
        'Please purchase the enrollment package First. Before purchasing monthly and per class packages. ',
    });
  };
  ...
  getParticularEntityFromMembersList = (entityType) => {
    ...
    const entities = membersList.filter((member) => {
      ...
      return entityType.indexOf(member.type) !== -1;
    });
    return entities;
  };
  ...
  updateStatus = (n, props) => {
    let {
      status, popUp, purchaseId, classData,
    } = props;
    const { scheduled_date } = (classData && classData) || {};
    let inc = 0;
    let packageType;
    let packageConnected = false;
    if (n === 1) {
    ...
    } else if (status === 'signIn' || status === 'checkIn') {
      if (status === 'checkIn') {
        inc = 1;
      }
      status = 'signOut';
    ...
    if (
      status == 'checkIn'
      && moment(scheduled_date).format('DD-MM-YYYY') > moment().format('DD-MM-YYYY')
    ) {
      let data = {};
      data = {
        popUp,
        ...
        content: (
          <div>
            You can't control future classes. Instructor not allowed to auto check in students for
            future classes.
          </div>
        ),
  ...
  handleDialogBoxState = (dialogState, currentProps, packagesRequired) => {
    this.setState(state => ({
      ...
      currentProps: currentProps || state.currentProps,
      packagesRequired,
    }));
  };
    ...
    popUp.appear(
      'inform',
      {
        ...
        onAffirmationButtonClick: () => {
          this.sendLink(props, (packageId = null), (packageType = null));
        },
        defaultButtons: true,
      },
      true,
  ...
  sendLink = (props, packageId = null, packageType = null) => {
    try {
      ...
      userName = get(
        props.profile,
        'firstName',
        get(props.profile, 'name', get(props.profile, 'lastName', 'Old Data')),
      );
  ...
  successPopUp = (popUp, userName, title) => {
    popUp.appear(
      ...
      {
        title: 'Success',
        content: title || `Email with purchase link send to ${userName} successfully.`,
        RenderActions: (
          <ButtonWrapper>
            <FormGhostButton
              label="Ok"
  ...
  confirmationPopUp = (popUp, data) => {
    popUp.appear(
      ...
      {
        title: 'Confirmation',
        content: `An email is already sent to ${
          data.userName
  ...
  acceptPaymentConfirmation = (packageData, props, paymentMethod) => {
    ...
    popUp.appear(
      'inform',
      {
        ...
        onAffirmationButtonClick: () => {
          this.acceptPayment(packageData, props, paymentMethod);
        },
        defaultButtons: true,
      },
      true,
    ...
    Meteor.call('classPricing.signInHandler', classDetails, (err, res) => {
      ...
      if (epStatus || packageType == 'EP') {
        Meteor.call('stripe.handleOtherPaymentMethods', data, (err, res) => {
          this.toggleIsBusy();
          const title = 'Package Purchased Successfully';
          if (packageType != 'EP') this.updateStatus(2, props);
          else this.successPopUp(popUp, null, title);
        });
      } else {
        this.toggleIsBusy();
        this.purchaseEnrollmentFirst(popUp);
  ...
  handleNoteChange = (doc_id) => {
    ...
    Meteor.call(
      ...
      { doc_id, doc: { adminNotes: this.state.notes } },
      (err, res) => {
        this.toggleIsBusy();
        const { popUp } = this.props;
        let data = {};
      ...
      <Fragment>
        ...
        {buyPackagesBoxState && (
          <BuyPackagesDialogBox
            ...
            onModalClose={() => {
              this.handleDialogBoxState(false);
            }}
        ...
        {notification && currentView === 'studentsView' && (
          <Notification
            notificationContent={`You need to purchase ${
              packagesRequired == 'enrollment' ? 'enrollment ' : 'monthly/per class '
            }package first.`}
            bgColor={danger}
        ...
        <ListWrapper>
          <MembersList
            ...
            addStudent
            onViewStudentClick={(userId) => {
              this.setState({ classTypePackages: true, userId });
            }}
            params={params}
