...
const Text = styled.p`
  ...
  transition: 0.2s linear max-width,
    ${props => (props.showComplete ? '0.1s linear opacity 0.15s' : '0s linear opacity')};
  font-family: ${helpers.specialFont};
  font-size: ${props => (props.largeFont ? helpers.baseFontSize * 1.25 : 18)}px;
  font-weight: 400;
  opacity: ${props => (props.showComplete ? 1 : 0)};
  padding: ${props => (props.showComplete
    ? props.longText
      ? `${helpers.rhythmDiv * 2}px`
      : `${helpers.rhythmDiv * 2}px ${helpers.rhythmDiv * 4}px`
    : 0)};
...
class ContactUsFloatingButton extends Component {
  ...
  render() {
    return (
      <Fragment>
        {this.state.contactUsDialogBox && (
          <ContactUsDialogBox
            open={this.state.contactUsDialogBox}
            onModalClose={() => this.handleContactUsDialogBox(false)}
          />
        )}
        <OuterWrapper>
          <Wrapper showComplete={this.state.showComplete}>
            <Text
              showComplete={this.state.showComplete}
              longText={this.state.showComplete}
              largeFont={false}
            >
              <MyText onClick={() => this.handleContactUsDialogBox(true)}>
                Confused ? Questions ? Click here
              </MyText>
              <Icon
                className={this.props.classes.iconStyles}
                onClick={this.handleShowCompleteButton}
              >
                {' '}
                keyboard_arrow_down
                {' '}
