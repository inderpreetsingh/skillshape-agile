...
class BuyPackagesDialogBox extends Component {
  ...
  packageSelectAlert = () => {
    ...
    popUp.appear('alert', {
      title: 'Alert',
      content: 'Package  not selected. Please select one package first then try again.',
    });
  };

  acceptPayment = () => {
    const { packageList, currentProps } = this.props;
    const { selectedPackageIndex, selectedPackageType, radioButtonGroupValue } = this.state;
    let packageData;
  ...
  handleSendLink = () => {
    ...
    if (selectedPackageType) {
      ...
      packageData = packageData[selectedPackageIndex];
      const { _id, packageType } = packageData;
      ...
      >
        <MuiThemeProvider theme={muiTheme}>
          <DialogTitle classes={{ root: classes.dialogTitleRoot }}>
            <DialogBoxTitleBar title="Packages" classes={classes} onModalClose={onModalClose} />
          </DialogTitle>
          <PastSubscriptions>
            <MyLink target="_blank" to={`${Meteor.absoluteUrl()}mySubscription/${userId}`}>
              Click here
              {' '}
            </MyLink>
            to view past subscriptions.
          </PastSubscriptions>
          {isLoading ? (
            <ContainerLoader />
          ) : (
          ...
          >
            <ActionButtons>
              ...
              <ActionButton>
                <PrimaryButton fullWidth label="Send Link" onClick={this.handleSendLink} />
              </ActionButton>
            </ActionButtons>
          </DialogActions>
        </MuiThemeProvider>
...
export default withStyles(styles)(
  createContainer((props) => {
    ...
    if (packagesRequired == 'perClassAndMonthly' && classTypeId) {
      monthlyPricingSubscription = Meteor.subscribe('monthlyPricing.getMonthlyPricingWithClassId', {
        classTypeId,
      });
      classPricingSubscription = Meteor.subscribe('classPricing.getClassPricingWithClassId', {
        classTypeId,
      });
      ...
      enrollmentSubscription = Meteor.subscribe('enrollmentFee.getEnrollmentFeeByClassTypeId', {
        classTypeId,
      });
    ...
    const CP = classPricingData.map((obj) => {
      obj.packageType = 'CP';
    ...
    const MP = monthlyPricingData.map((obj) => {
      obj.packageType = 'MP';
      return obj;
    });
    const enrollmentFeeData = EnrollmentFees.find().fetch();
    const EP = enrollmentFeeData.map((obj) => {
      obj.packageType = 'EP';
      return obj;
    });
    const packageList = [];
