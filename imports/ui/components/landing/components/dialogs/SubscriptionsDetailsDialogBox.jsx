...
import {
  calcRenewalDate, capitalizeString, formatDate, formatMoney,
} from '/imports/util';
  ...
  const getRemainingClasses = (props) => {
    ...
    if (props.combinedData) {
      props.combinedData.map((obj, index) => {
        stringToPrint += ` ${obj.noClasses} ${
          obj.noClasses <= 1 ? 'Class' : 'Classes'
        } expires on ${formatDate(obj.endDate)} <br/>`;
      });
    } else {
      stringToPrint += ` ${props.noClasses} ${
        props.noClasses <= 1 ? 'Class' : 'Classes'
      } expires on ${formatDate(props.endDate)} <br/>`;
    }
    ...
    if (props.autoWithdraw || props.payUpFront || props.payAsYouGo) {
      return (
        <Text>
          <b>Contract ends:</b>
          {' '}
          {calcRenewalDate(props.endDate, props.packageType === 'MP', props.contractLength)}
        </Text>
      );
    }
  };
...
of
            ...
            {calcRenewalDate(
              ...
              props.combinedData && props.combinedData.length - 1,
            )}
          </Text>
          {getContractEnds()}
        </HighlightedBg>
    ...
    if (props.payUpFront) {
      ...
      contractLength = props.combinedData && props.combinedData.length > 1
        ? contractLength * props.combinedData.length - 1
        : 0;
      return (
        <HighlightedBg>
          <Text>
            <b>Paid until</b>
            {' '}
            {calcRenewalDate(props.endDate, props.packageType === 'MP', contractLength)}
          </Text>
          {getContractEnds()}
        </HighlightedBg>
      );
    }
    if (props.packageType == 'MP') {
      return (
        <HighlightedBg>
          <Text>
...
will process
            {' '}
            {calcRenewalDate(props.endDate, props.packageType === 'MP', 0)}
          </Text>
          {getContractEnds()}
        </HighlightedBg>
      );
  ...
  const {
    open, classes, fullScreen, onModalClose, contractDialog, toggleContractDialog,
  } = props;
      ...
      <MuiThemeProvider theme={muiTheme}>
        ...
        <DialogContent classes={{ root: classes.dialogContent }}>
          <ContentWrapper>
            ...
            {props.packageType == 'CP' && (
              <HighlightedBg>{ReactHtmlParser(getRemainingClasses(props))}</HighlightedBg>
            )}

            {/* Depending upon the type of payment method */}
            {getDatesBasedOnSubscriptions(props)}

            {classesCovered.length > 0 && (
              <ClassesCovers>
                <ContentHead>This Covers:</ContentHead>
                <ClassesList>
