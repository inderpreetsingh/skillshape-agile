...
class AddInstructorDialogBox extends Component {
      ...
      'inform',
      {
        ...
        RenderActions: (
          <ActionButtons>
            <ActionButton>
              <FormGhostButton
                label="Just to this instance"
                onClick={() => {
                  this.handleInstructors(popUp, payLoad);
                }}
                applyClose
              />
            </ActionButton>
            <ActionButton>
              <FormGhostButton
                label="Whole series"
                onClick={() => {
                  this.handleWholeSeries(popUp, payLoad);
                }}
                applyClose
              />
            </ActionButton>
  ...
  handleWholeSeries = (popUp, payLoad) => {
    ...
    Meteor.call(
      'classTimes.editClassTimes',
      { doc_id: payLoad.classTimeId, doc: payLoad },
      (err, res) => {
              ...
              RenderActions: (
                <ActionButtons>
                  <FormGhostButton
                    label="Ok"
                    onClick={() => {
                      this.props.instructorsIdsSetter
                        ? this.props.instructorsIdsSetter(payLoad.instructors, 'add')
      ...
      if (res != 'emailNotFound') {
        popUp.appear(
          'success',
          {
            ...
            content: (
              <div>
                Successfully added as an instructor.
                <br />
                {payLoad.classTimeForm
                  ? 'Changes will show in the Class Times Editor after saving.'
                  : ''}
              </div>
            ),
            RenderActions: (
  ...
  onSubmit = (e) => {
    ...
    const payLoad = {
      ...
      classTypeId: !isEmpty(this.props.classData)
        ? get(this.props.classData, 'classTypeId', '')
        : '',
    ...
    if (this.props.text == 'Student') {
      ...
      (payLoad.instructors = !isEmpty(this.props.classData)
        ? get(this.props.classData, 'instructors', this.props.instructorsIds)
        : []),
        ...
        RenderActions: (
          <ActionButtons>
            ...
            <FormGhostButton
              label="Yes"
              onClick={() => {
                this.props.text == 'Student'
                  ? this.handleAddStudent(popUp, payLoad)
                  : !payLoad.classTimeForm
                    ? this.handleClassesAndClassTime(popUp, payLoad)
                    : this.handleInstructors(popUp, payLoad);
              }}
              applyClose
            />
  ...
  render() {
    const { props } = this;
    const { selectedOption, emailList, isLoading } = this.state;
    const text = props.text ? props.text : 'Instructor';
    return (
      <Dialog
        open={props.open}
            ...
            <form id="addUser" onSubmit={this.onSubmit}>
              ...
              <ActionButtons>
                ...
                {this.state.selectedOption.length ? (
                  <PrimaryButton formId="addUser" type="submit" label={`Add new ${text}`} />
                ) : (
                  <SecondaryButton disabled label={`Add new ${text}`} />
                )}

                <PrimaryButton formId="cancelUser" label="Cancel" onClick={props.onModalClose} />
              </ActionButtons>
            </form>
          </DialogContent>
