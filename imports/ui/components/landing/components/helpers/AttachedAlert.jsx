...
const InnerWrapper = styled.div`
  ...
  width: 100%;
  background: ${props => (props.open ? props.color || information : 'transparent')};
...
class AttachedAlert extends Component {
  ...
  render() {
    ...
    const {
      bgColor, classes, iconName, alertMsg, direction,
    } = this.props;
    const alertOpenIconClasses = [
      classes.icon,
      classes.isOpen,
      direction === 'right' ? classes.iconRightSide : classes.iconLeftSide,
    ].join(' ');
    const alertCloseIconClases = [classes.icon, classes.isClosed].join(' ');
    return (
      <Wrapper>
        <InnerWrapper open={open} color={bgColor} direction={direction}>
          <Icon className={open ? alertOpenIconClasses : alertCloseIconClases}>{iconName}</Icon>
          <CSSTransition in={open} timeout={50} classNames="message" unmountOnExit>
