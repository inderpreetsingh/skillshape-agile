...
class Upload extends React.Component {
  ...
  onInputChange = (e) => {
    const org = e.target.files[0];
    filter(e.target.files, file => file.type.match(this.props.fileTypeRegex) !== null).forEach(
      (file) => {
  ...
  componentDidMount() {
    findDOMNode(this.uploadInput).addEventListener('change', this.onInputChange, false);
  }

  componentWillUnmount() {
    findDOMNode(this.uploadInput).removeEventListener('change', this.onInputChange, false);
  }
  ...
  render() {
    const { classes, fullScreen, minWidth } = this.props;
