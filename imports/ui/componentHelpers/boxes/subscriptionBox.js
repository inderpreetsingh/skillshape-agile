                ...
                return (
                  <div key={index.toString()}>
                    <Paper className={classes.singleSubscription}>
                      ...
                      <Expiring>
                        {status == 'inActive' ? 'Starting' : 'Expiring'}
                        {' '}
                        {moment(current.endDate).format('Do MMMM YYYY')}
                      </Expiring>
                      {get(current, 'packageType', 'MP') == 'MP' && (
                        <Expiring>
                          Renewal On
                          {' '}
                          {moment(current.endDate)
                            .add(1, 'M')
                            .format('Do MMMM YYYY')}
                        </Expiring>
                      )}
