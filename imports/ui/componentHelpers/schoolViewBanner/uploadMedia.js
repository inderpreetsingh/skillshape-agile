        ...
        if (_.isArray(result)) {
          ...
          Promise.all(allUploadPromise).then(() => {
            if (file && file.isUrl) {
              ...
              S3.upload({ files: { 0: file && file.fileData }, path: 'schools' }, (err, res) => {
                if (res) {
          ...
          if (file && file.isUrl) {
            ...
            S3.upload(
              { files: { 0: file && file.fileData && file.fileData }, path: 'schools' },
              (err, res) => {
                if (res) {
                  doc[this.props.imageType] = res.secure_url;
                  this.handleSubmit(doc);
  ...
  render() {
    ...
    return (
      ...
      >
        <form
          onSubmit={
            forClassType ? this.onSubmitForClassTypeData(schoolId, classTypeId) : this.onSubmit
          }
        >
