...
class ChipInput extends React.Component {
  ...
  componentWillMount() {
    const { name, hintText, floatingLabelText } = this.props;
  ...
  handleInputBlur = event => {
    ...
    setTimeout(() => {
      if (
        this.autoComplete &&
        (!this.autoComplete.state.open ||
          this.autoComplete.requestsList.length === 0)
      ) {
      ...
      if (this.props.dataSourceConfig) {
        ...
        if (changed) {
          this.setState({
            ...
            focusedChip:
              this.state.focusedChip &&
              this.state.focusedChip[this.props.dataSourceConfig.value] === chip
                ? null
                : this.state.focusedChip
          });
          if (this.props.onChange) {
        ...
        if (changed) {
          this.setState({
            chips,
            focusedChip:
              this.state.focusedChip === chip ? null : this.state.focusedChip
          });
          if (this.props.onChange) {
            this.props.onChange(chips);
          }
  ...
  render() {
    ...
    const autoCompleteData = dataSourceConfig
      ? (dataSource || []).filter(
          value =>
            !chips.some(
              c => c[dataSourceConfig.value] === value[dataSourceConfig.value]
            )
        )
    ...
    return (
      <div
        className={className}
        style={prepareStyles(
          Object.assign(styles.root, style, overrideRootStyles)
        )}
        onClick={() => this.focus()}
      >
        <div>
          ...
          <div
            style={Object.assign(styles.chipContainer, chipContainerStyle, {
              fontFamily: commonFont
            })}
          >
            {chips.map((tag, i) => {
              const value = dataSourceConfig
                ? tag[dataSourceConfig.value]
                : tag;
              return chipRenderer(
                {
                  value,
                  ...
                  chip: tag,
                  isDisabled: disabled,
                  isFocused: dataSourceConfig
                    ? this.state.focusedChip &&
                      this.state.focusedChip[dataSourceConfig.value] === value
                    : this.state.focusedChip === value,
